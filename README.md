...
> Current branch note: SQLite configuration, `doctor`, and `init` are wired for the v0.4.2 integration point. The SQLite store/schema itself is owned by Worker A, so `proxyharbor init -storage=sqlite` currently reports a clear coming-soon message instead of pretending to initialize data.
...
Run the local diagnostics before starting or when troubleshooting:
