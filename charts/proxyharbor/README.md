...
For HA, use `examples/dynamic-ha-values.yaml` as the stable v0.5.4 starting point. It keeps MySQL as shared storage, Redis as the required zfair/cache coordination layer, and enables the cluster heartbeat/maintenance loop used by the current HA correctness runners.
