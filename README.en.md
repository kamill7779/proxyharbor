...
- **Global proxy inventory**: Admin manages Providers and Proxies; tenants receive leases and do not read proxy endpoint lists.
...
This is the shortest all-default loop: connect to `http://localhost:18080` by default, read the local Admin Key from `data/secrets.env`, add one proxy with the SDK, then get a proxy URL ready for an HTTP client. Replace the sample endpoint with your real proxy; the example uses an IP literal so local DNS pollution does not break the docs path.
...
> For loopback or private-network proxy endpoints in local testing, set `PROXYHARBOR_ALLOW_INTERNAL_PROXY_ENDPOINT=true` before startup. This switch only relaxes proxy endpoint registration; it does not relax SSRF checks for lease targets.
...
### Local HA verification path

To repeatedly validate the local 3 instance + MySQL + Redis + LB HA topology, use the formal runners instead of ad-hoc scripts. See the [v1.0.0 release runbook](docs/runbooks/release-v1.0.0.md) for the full release-candidate matrix; the README keeps only the core entry points:
...
`-docker-internal` runs the pressure worker inside the compose network and avoids host port-publishing connection-refusal noise on Docker Desktop / Windows / macOS. See the [HA pressure runbook](docs/runbooks/ha-pressure.md) for complete HA pressure commands, per-operation checks, and result formatting.

Current HA hot-path availability evidence from the local Docker topology (3 ProxyHarbor instances + MySQL + Redis + nginx, `-docker-internal`):
...
These numbers measure only control-plane hot paths such as lease creation, renewal, and gateway validation. They do not represent external proxy data-plane throughput. The `500` concurrency, `10m` mixed-soak availability gate is met; strict per-operation p95/p99 latency gates remain a P1 performance follow-up. See the full evidence in [v0.5.5 notes](docs/versions/v0.5.5.md) and [v1.0.0 readiness](docs/versions/v1.0.0.md).
...
Issues and PRs are welcome. Read [CONTRIBUTING.md](CONTRIBUTING.md) before opening a change. Single-node capabilities should stay lightweight, reproducible, and easy to verify. HA capabilities should keep explicit secrets and clear cloud-native deployment boundaries.
